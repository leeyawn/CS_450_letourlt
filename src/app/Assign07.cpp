...
void keyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if(action == GLFW_PRESS || action == GLFW_REPEAT) {
        if(key == GLFW_KEY_ESCAPE) {
            ...
            metallic -= 0.1f;
            metallic = glm::max(0.0f, metallic);
		}
        else if(key == GLFW_KEY_B) {
            metallic += 0.1f;
            metallic = glm::min(1.0f, metallic);
		}
		else if(key == GLFW_KEY_N) {
            roughness -= 0.1f;
            roughness = glm::max(0.1f, roughness);
		}
		else if(key == GLFW_KEY_M) {
            roughness += 0.1f;
            roughness = glm::min(0.7f, roughness);
		}
	}
}
...
int main(int argc, char **argv) {
	...
	GLFWwindow* window = setupGLFW("assign07: letourlt", 4, 1, 800, 800, DEBUG_MODE);
